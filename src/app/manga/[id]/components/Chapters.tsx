...
import { checkVolumeCompleteness } from '@/app/lib/checkVolumeCompleteness';
...
export default async function Chapters({ id }: ChaptersProps) {
    const mangaAggregateResponse = await fetch(`${baseURL}/api/manga/${id}/aggregate`);
                                        ...
                                        .map((chapterKey) => {
                                            ...
                                            return (
                                                <li key={chapterKey} className="flex items-center my-1.5">
                                                    <Link href={`/manga/${id}/chapter/${chapter.id}`} className="hover:text-orange-600">
