export function checkVolumeCompleteness(chapters: string[]): VolumeCheckResult {
    ...
    const chapterNumbers: number[] = chapters.map((ch) => parseFloat(ch.replace('Chapter ', '')));

    // Sort chapters numerically
    chapterNumbers.sort((a, b) => a - b);

    const uniqueWholeChapters = new Set<number>(); // Store unique whole number chapters
    const allWholeChapters: number[] = []; // Sorted list of all whole-number chapters found

    // Add whole number chapters to the set
    chapterNumbers.forEach((ch) => {
        const wholeNumber = Math.floor(ch);
    ...
    for (let i = 1; i < allWholeChapters.length; i++) {
        const expectedNext = allWholeChapters[i - 1] + 1;
    ...
    const expectedChapterCount = chapterNumbers.length + missingChapters.length;
